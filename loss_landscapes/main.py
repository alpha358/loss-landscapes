...
# noinspection DuplicatedCode
def random_plane(model: typing.Union[torch.nn.Module, ModelWrapper], metric: Metric, DEVICE, distance=1, steps=20,
                 normalization='filter', deepcopy_model=False) -> np.ndarray:
    """
    ...
    start_point = model_start_wrapper.get_module_parameters().to(DEVICE)
    dir_one = rand_u_like(start_point).to(DEVICE)
    dir_two = orthogonal_to(dir_one).to(DEVICE)
